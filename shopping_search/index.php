...
(function($) {
    ...
    function getTotalPages(){
        myWait.show();
        $.get("http://198.199.67.218/shopping_search/total_pages.txt")
        // The '.done' method fires when the get request completes
        .done(function(data) {
            var total_pages = data;
            $.get("http://198.199.67.218/api/api.php/products?order=id"+"&page=" + 1 + "," + 10)
            .done(function(data) {
            ...
            $('#search-btn').click(function(event){
                ...
                search_key = $('#search-keys').val();
                $.get("http://198.199.67.218/api/api.php/products?filter=desc,cs,"+search_key)
                // The '.done' method fires when the get request completes
                .done(function(data) {
                    total_records = data.products.records.length;
                    total_pages = parseInt(total_records / page_size);
                    
                    $.get("http://198.199.67.218/api/api.php/products?filter=desc,cs,"+search_key + "&order=id" + "&page=" + page + "," + 10)
                    .done(function(data){
                    loadTableData(data);
                    });  
                });
            ...
            function pagination(x){
                ...
                $('#pagination-demo').twbsPagination({
                    ...
                    onPageClick: function (event, page) {
                        ...
                        page_size = 10;
                        if(x === "nosearch"){
                            var searchUrl = "http://198.199.67.218/api/api.php/products?order=id"+"&page=" + page + "," + page_size
                        }
                        else{
                            searchUrl = "http://198.199.67.218/api/api.php/products?filter=desc,cs,"+search_key + "&order=id" + "&page=" + page + "," + 10
	...
	function addCartItem(item){
		...
		$.post("http://198.199.67.218/api/api.php/shopping_cart/",postData);
	}
